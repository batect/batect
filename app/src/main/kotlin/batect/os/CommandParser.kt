...
class CommandParser {
    fun parse(command: String?): Iterable<String> {
        if (command == null) {
            return emptyList()
        }
    ...
    private fun invalidCommandLine(command: String, message: String): Throwable = InvalidCommandLineException("Command line `$command` is invalid: $message")
