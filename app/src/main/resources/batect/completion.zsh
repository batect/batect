PLACEHOLDER_REGISTER_AS() {
    ...
    local -a args
    args=(
        '--cache-type=:value:(directory volume)'
        '--clean'
        '(-f)--config-file=:file name:_files'
        '*--config-var=::( )'
        '--config-vars-file=:file name:_files'
        '--disable-ports'
        '--docker-cert-path=:directory name:_files -/'
        '--docker-config=:directory name:_files -/'
        '--docker-host=::( )'
        '--docker-tls'
        '--docker-tls-ca-cert=:file name:_files'
        '--docker-tls-cert=:file name:_files'
        '--docker-tls-key=:file name:_files'
        '--docker-tls-verify'
        '--enable-buildkit'
        '--help'
        '--linux-cache-init-image=::( )'
        '(-T)--list-tasks'
        '--log-file=:file name:_files'
        '--max-parallelism=::( )'
        '--no-cleanup'
        ...
        '(-o)--output=:value:(all fancy quiet simple)'
        '*--override-image=::( )'
        ...
        '--version'
        '(--list-tasks)-T'
        '(--config-file)-f=:file name:_files'
        '(--output)-o=:value:(all fancy quiet simple)'
