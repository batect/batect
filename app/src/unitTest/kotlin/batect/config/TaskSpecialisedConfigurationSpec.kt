    ...
    describe("task-specialised configuration") {
        describe("converting it to JSON for logging") {
            given("a single task") {
                ...
                val json by createForEachTest { Json.forLogging.encodeToString(TaskSpecialisedConfiguration.serializer(), configuration) }
            ...
            given("a single config variable") {
                ...
                val configuration = TaskSpecialisedConfiguration("the-project", TaskMap(), ContainerMap(), ConfigVariableMap(configVariable))
                val json by createForEachTest { Json.forLogging.encodeToString(TaskSpecialisedConfiguration.serializer(), configuration) }
