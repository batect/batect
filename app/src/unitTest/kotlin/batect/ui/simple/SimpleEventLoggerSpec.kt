    ...
    describe("a simple event logger") {
        ...
        describe("handling when events are posted") {
            ...
            on("when an 'image built' event is posted") {
                beforeEachTest {
                    val event = ImageBuiltEvent(otherContainer, ImageReference("abc-123"))
                    logger.postEvent(event)
                }
            ...
            on("when an 'image pulled' event is posted") {
                beforeEachTest {
                    val event = ImagePulledEvent(PullImage("the-cool-image:1.2.3"), ImageReference("the-cool-image-id"))
                    logger.postEvent(event)
                }
