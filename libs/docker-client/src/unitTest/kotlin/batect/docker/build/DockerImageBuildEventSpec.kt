        ...
        describe("build progress events") {
            describe("converting to a human readable string") {
                given("the event has no active steps") {
                    given("the event has a single step to run in total") {
                        val event = BuildProgress(emptySet(), 1)

                        it("returns a description of the total number of steps") {
                            assertThat(event.toHumanReadableString(), equalTo("1 step total"))
                        }
                    }

                    given("the event has multiple steps to run in total") {
                        val event = BuildProgress(emptySet(), 3)

                        it("returns a description of the total number of steps") {
                            assertThat(event.toHumanReadableString(), equalTo("3 steps total"))
                        }
                    }
                }
                ...
                given("the event has a single active step") {
                    given("the step is not downloading") {
                        val event = BuildProgress(setOf(ActiveImageBuildStep.NotDownloading(1, "RUN blah.sh")), 3)
                    ...
                    given("the step is downloading") {
                        ...
                        setOf(
                            TestCase(
                                ...
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.Downloading, 0, 200),
                                "step 2 of 3: FROM postgres:13.0: downloading: 0 B of 200 B (0%)"
                            ),
                            TestCase(
                                "the download is for a file larger than 1 KB",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.Downloading, 1500, 2000),
                                "step 2 of 3: FROM postgres:13.0: downloading: 1.5 KB of 2.0 KB (75%)"
                            ),
                            TestCase(
                                "the download is for a file larger than 1 MB",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.Downloading, 1_750_000, 2_000_000),
                                "step 2 of 3: FROM postgres:13.0: downloading: 1.8 MB of 2.0 MB (88%)"
                            ),
                            TestCase(
                                "the download is for a file larger than 1 GB",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.Downloading, 1_750_000_000, 2_000_000_000),
                                "step 2 of 3: FROM postgres:13.0: downloading: 1.8 GB of 2.0 GB (88%)"
                            ),
                            TestCase(
                                "the download is for a file larger than 1 TB",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.Downloading, 1_750_000_000_000, 2_000_000_000_000),
                                "step 2 of 3: FROM postgres:13.0: downloading: 1.8 TB of 2.0 TB (88%)"
                            ),
                            TestCase(
                                "the download has finished",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.Downloading, 200, 200),
                                "step 2 of 3: FROM postgres:13.0: downloading: 200 B of 200 B (100%)"
                            ),
                            TestCase(
                                "the download is verifying the checksum",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.VerifyingChecksum, 50, 200),
                                "step 2 of 3: FROM postgres:13.0: verifying checksum: 50 B of 200 B (25%)"
                            ),
                            TestCase(
                                "the download is extracting",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.Extracting, 50, 200),
                                "step 2 of 3: FROM postgres:13.0: extracting: 50 B of 200 B (25%)"
                            ),
                            TestCase(
                                "the download is complete",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.DownloadComplete, 200, 200),
                                "step 2 of 3: FROM postgres:13.0: download complete: 200 B of 200 B (100%)"
                            ),
                            TestCase(
                                "the download is for an image pull that is complete",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.PullComplete, 200, 200),
                                "step 2 of 3: FROM postgres:13.0: pull complete: 200 B of 200 B (100%)"
                            ),
                            TestCase(
                                "the download has an invalid total size",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.Downloading, 200, 0),
                                "step 2 of 3: FROM postgres:13.0: downloading: 200 B"
                            ),
                            TestCase(
                                "the download has no total size",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.Downloading, 200, null),
                                "step 2 of 3: FROM postgres:13.0: downloading: 200 B"
                            ),
                            TestCase(
                                "the download has no total size and has not started",
                                ActiveImageBuildStep.Downloading(1, "FROM postgres:13.0", DownloadOperation.Downloading, 0, null),
                                "step 2 of 3: FROM postgres:13.0: downloading"
                            ),
                        ).forEach { testCase ->
                            given(testCase.description) {
                                val event = BuildProgress(setOf(testCase.step), 3)
                ...
                given("the event has two active steps") {
                    val event = BuildProgress(
                        setOf(
                            ActiveImageBuildStep.NotDownloading(1, "RUN blah.sh"),
                            ActiveImageBuildStep.NotDownloading(4, "RUN foo.sh"),
                        ),
                        30
                    )
                ...
                given("the event has three active steps") {
                    val event = BuildProgress(
                        setOf(
                            ActiveImageBuildStep.NotDownloading(1, "RUN blah.sh"),
                            ActiveImageBuildStep.NotDownloading(4, "RUN foo.sh"),
                            ActiveImageBuildStep.NotDownloading(9, "RUN bar.sh"),
                        ),
                        30
                    )
