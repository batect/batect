    ...
    describe("a value option") {
        describe("creation") {
            ...
            on("attempting to create an value option with a valid name, description and short name") {
                it("does not throw an exception") {
                    assert.that({ ValueOption("value", "The value.", 'v') }, !throws<Throwable>())
                }
            }
            ...
            on("attempting to create an value option with a short name that is a dash") {
                it("does not throw an exception") {
                    assert.that({ ValueOption("value", "The value.", '-') }, throws<IllegalArgumentException>(withMessage("Option short name must be alphanumeric.")))
                }
            }
