    ...
    describe("a 'container creation failed' event") {
        ...
        on("getting the message to display to the user") {
            it("returns an appropriate message") {
                assert.that(event.messageToDisplay, equalTo("Could not start container 'container-1': Something went wrong"))
            }
        }

        on("toString()") {
            it("returns a human-readable representation of itself") {
                assert.that(event.toString(), equalTo("ContainerStartFailedEvent(container: 'container-1', message: 'Something went wrong')"))
            }
        }
    }
})
